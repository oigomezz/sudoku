...
function isValidPuzzle(grid) {
  return Utils.isValidPuzzle(grid);
}
    ...
    for (let col = 0; col < 9; col++) {
      if (grid[row][col] === 0) {
        for (let possibleNumber = 1; possibleNumber <= 9; possibleNumber++) {
          if (Utils.isValidPlace(grid, row, col, possibleNumber)) {
...
function getRandomSudoku() {
  const randomSudoku = [];
  for (let i = 0; i < 9; i++) randomSudoku[i] = Array(9).fill(0);

  for (let i = 0; i < 8; i++) {
    let number = Math.floor(Math.random() * 8) + 1;
    while (!Utils.isValidPlace(randomSudoku, 0, i, number))
      number = Math.floor(Math.random() * 8) + 1;

    if (Utils.isValidPlace(randomSudoku, 0, i, number))
